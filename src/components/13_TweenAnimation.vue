...
let scene, camera, renderer, cube, animationId, controls, tween, tween2;
// 初始化场景
function initScene() {
  // 创建场景
...
function initGui() {
  ...
  tween = new TWEEN.Tween(cube.position)
    .to({ x: 5 }, 2000) // 2000ms 内将球移动到 x=5 的位置
    .easing(TWEEN.Easing.Quadratic.InOut) // 缓动函数
    // .repeat(Infinity) // 无限循环
    .yoyo(false); // 动画完成后反向播放

  // 创建另一个补间动画
  tween2 = new TWEEN.Tween(cube.position).to({ x: -5 }, 2000); // 2000ms 内将球移动到 y=5 的位置
  ...
  gui.add(actions, "start").name("开始动画");
  gui.add(actions, "stop").name("停止动画");
}
