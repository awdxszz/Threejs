...
function addTrackballController() {
  ...
  controls.minPolarAngle = Math.PI / 2 - Math.PI / 12; // 最小垂直角度
  controls.maxPolarAngle = Math.PI / 2; // 最大垂直角度
  controls.minAzimuthAngle = Math.PI / 2 - Math.PI / 12; // 最小水平角度
  controls.maxAzimuthAngle = Math.PI / 2 + Math.PI / 12; // 最大水平角度
  controls.update();
}
...
function initGui() {
  ...
  // 加载模型
  loader.load(
    new URL(`/model/liveroom-scene.glb`, import.meta.url).href,
    (gltf) => {
